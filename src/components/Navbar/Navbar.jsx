...
const Navbar = () => {
	...
	const generateRandomURL = () => {
		const random64BitHexCode = Array.from({ length: 32 }, () =>
	...
	const navigationItems = (
		<>
			<NavLink onClick={() => setOpenNavbar(false)} className={navClasses} to={'/'}>
			...
			<NavLink
				...
				to={'/packages'}
			>
				<RiNpmjsLine className="sm:hidden" />
				Packages
			</NavLink>
		</>
	);
		...
		logOut()
			...
			.catch((error) => {
				Swal.fire({
					title: 'Error!',
					text:
						error?.message.split(': ')[1] || 'Error Occurred! Try Again Later!',
					icon: 'error',
					confirmButtonText: 'Close',
					color: '#fff',
					background: '#05030efc',
			...
			>
				...
				<ul
					className={`w-3/5 sm:w-full flex flex-col sm:flex-row justify-start sm:justify-center gap-2 sm:gap-5 text-base sm:text-lg font-semibold transition-all duration-700 absolute sm:static sm:shadow-none h-auto pl-6 p-4 sm:p-0 ${
						openNavbar
							? 'shadow-md shadow-blue-500 right-3 -bottom-32 bg-nhb bg-opacity-80 flex z-10'
							: 'right-3 bottom-full shadow-none'
					}`}
				>
					<div className="absolute inset-0 backdrop-filter backdrop-blur-lg -z-10"></div>
					{navigationItems}
