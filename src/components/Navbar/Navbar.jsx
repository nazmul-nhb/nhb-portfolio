...
const Navbar = () => {
    ...
    return (
        <nav className="max-w-screen-2xl bg-nhbBG text-white flex items-center justify-between gap-0 md:gap-4 mx-auto shadow-lg shadow-nhb px-6 py-3 md:px-12 sticky top-0 bg-opacity-75 h-16 z-20">
            <figure className="flex items-center justify-start gap-2 font-kreonSerif">
                <img className="w-11 h-11 rounded-full p-0.5 border" src={profile} alt="profile" />
            ...
            <div
                ...
                />
                <ul
                    className={`w-3/5 sm:w-full flex flex-col sm:flex-row justify-start sm:justify-center gap-2 sm:gap-5 text-base sm:text-lg font-semibold duration-1000 absolute sm:static shadow-lg shadow-nhb sm:shadow-none h-screen sm:h-auto pl-6 p-4 sm:p-0 ${openNavbar ? 'left-0 top-16 bg-nhbBG flex z-10' : '-left-full top-16'}`}>
                    {navigationItems}
                </ul>
            </div>
        </nav>
