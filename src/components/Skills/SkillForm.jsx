...
const SkillForm = ({ handleAddNewSkill }) => {
	...
	return (
		...
		>
			<h3 className="text-lg md:text-3xl lg:text-2xl xl:text-3xl text-center mb-4 animate-bounce font-bold">
				Add A New Skill
			</h3>
			...
			<div className="flex items-center gap-2 rounded-lg bg-transparent border-blue-200 border shadow-md shadow-blue-500">
				<label
					htmlFor="serial"
					className="flex items-center gap-1 pl-2 md:text-lg"
				>
					<FaSlackHash /> Serial
				</label>
				<input
			...
			<div className="flex items-center gap-2 rounded-lg bg-transparent border-blue-200 border shadow-md shadow-blue-500">
				...
				<input
					{...register("level", {
						...
						min: {
							value: 0.0,
							message: "Skill Level cannot be a negative value!",
						},
						max: {
							value: 100.0,
					...
					id="level"
					type="number"
					placeholder="Skill Level (Between 0 & 100)"
					className="px-2 rounded-r-lg py-2 w-full border-l bg-transparent focus:outline-0 text-white"
				/>
			</div>
			...
			<button
				type="submit"
				className="absolute top-4 right-8 text-3xl text-teal-500 hover:text-blue-50 hover:scale-110 animate-growShrink transition-all duration-500"
			>
				<FaRegSave />
			</button>
		</form>
