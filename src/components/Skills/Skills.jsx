...
const Skills = ({ updateSkill }) => {
	...
	const handleUpdateSkill = (id, updatedSkill) => {
		...
		try {
			axiosSecure
				.patch(`/skills/update/${id}`, updatedSkill)
				.then((res) => {
	...
	return (
		<section
			...
			ref={ref}
			className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-12"
		>
			{skills.map((skill) => (
				<div
					key={skill._id}
				...
				>
					...
					{showUpdateForm !== skill._id ? (
						<div className="flex-1 font-kreonSerif">
							<h3 className="text-lg font-semibold flex items-center justify-between group-hover:text-xl transition-all duration-1000">
								...
								<span className="hidden">
									{(hoveredSkillId === skill._id ||
										viewedSkills[skill._id]) && (
										<CountUp
											key={viewedSkills[skill._id]}
											delay={0.1}
											start={0}
											end={skill.level}
											duration={2}
						...
						// update form
						<form
							onSubmit={handleSubmit((skillData) =>
								handleUpdateSkill(skill?._id, skillData)
							)}
						...
						>
								...
								<input
									...
									{...register("level", {
										...
										min: {
											value: 0.0,
											message:
												"Skill Level cannot be a negative value!",
										},
										max: {
											value: 100.0,
											message:
												"Skill Level cannot exceed 100!",
										},
									})}
									name="level"
									id="level"
							...
							<button
								title="Save"
								type="submit"
								className="absolute -top-6 -left-20 text-3xl text-teal-500 hover:text-blue-50 hover:scale-105 transition-all duration-500"
							>
								<FaSave />
							</button>
						</form>
					...
					{updateSkill && (
						<div className="absolute -top-1 -right-3 -bottom-1 flex flex-col justify-between gap-4">
							...
							<MdDeleteForever
								onClick={() =>
									handleDeleteSkill(skill?._id, skill?.title)
								}
