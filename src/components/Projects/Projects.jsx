			...
			{projects?.map((project) => (
				...
				>
					...
					>
						...
						/>
						{project?.title}
						<span className="text-xs border px-1 group-hover:border-blue-500">
							{project?.type}
						</span>
					</h3>
					<figure
					...
					{openProjectID === project?._id && (
						<>
							...
							<dialog
								...
								} w-[96%] md:w-3/4 lg:w-3/4 xl:w-3/5 2xl:w-auto h-3/4 bg-opacity-95 p-2 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-transparent rounded-lg z-50 text-white`}
							>
									...
									{!showUpdateForm ? (
										<div className="flex flex-col gap-4 h-3/4 p-1 sm:p-6 pb-6 text-sm sm:text-base">
											...
											<div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4 font-semibold">
												...
												<a
													href={project?.link}
													className="border px-3 border-white rounded-3xl hover:text-nhb hover:bg-white transition-all duration-500 hover:animate-glowBorder shadow-md shadow-blue-400"
													target="_blank"
